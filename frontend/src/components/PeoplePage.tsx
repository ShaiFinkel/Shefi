...
export function PeoplePage() {
  ...
  const countries = useMemo(() => {
    ...
    for (const e of employees) if (e.country) set.add(e.country);
  ...
  }, [employees, search, filterCountry]);
  ...
  async function depart(id: number, name: string) {
    const date = prompt(`תאריך עזיבה של ${name} (YYYY-MM-DD):`, new Date().toISOString().slice(0, 10));
    if (!date) return;
    setBusy(true);
    try {
      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-ink2 mt-1">
              {filtered.length} מתוך {employees.length} · {showInactive ? "כולל מי שעזב/ה" : "פעילים בלבד"}
            </p>
          </div>
          <button
          ...
          <div className="bg-panel border border-accent/50 rounded-xl p-4 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <Field label="יום הולדת (MM-DD)">
                <input
                  value={form.birthday_md}
                  onChange={(e) => setForm({ ...form, birthday_md: e.target.value })}
                  placeholder="06-15"
                  className="bg-bg border border-panel2 rounded px-2 py-1 w-full"
                />
              </Field>
              <Field label="סכום (₪)">
                <input
                  ...
                  onChange={(e) => setForm({ ...form, amount_ils: Number(e.target.value) })}
                  className="bg-bg border border-panel2 rounded px-2 py-1 w-full"
                />
              </Field>
              <Field label="ערוץ">
            ...
            <div className="flex justify-end gap-2 mt-3">
              <button
                onClick={() => { setAdding(false); setForm(DEFAULT_NEW); }}
                className="text-sm px-4 py-2 text-ink2 hover:text-ink"
              >
                ביטול
              </button>
              <button
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <input
            ...
            className="bg-panel border border-panel2 rounded-md px-3 py-1.5 text-sm flex-1 min-w-[200px]"
          />
          <select
            value={filterCountry}
            onChange={(e) => setFilterCountry(e.target.value)}
            className="bg-panel border border-panel2 rounded-md px-3 py-1.5 text-sm"
          >
          ...
          <table className="w-full text-sm">
            <thead className="bg-panel2/50 text-xs text-ink2 uppercase">
              <tr>
                ...
                <th className="text-right px-3 py-2">מדינה</th>
            ...
            <tbody>
                ...
                return (
                  <tr key={e.id} className={`border-t border-panel2 ${e.active === 0 ? "opacity-50" : "hover:bg-panel2/30"}`}>
                    ...
                    <td className="px-3 py-2 text-ink2">
                      {isEditing ? (
                        <input value={editForm.country ?? ""} onChange={(ev) => setEditForm({ ...editForm, country: ev.target.value })} className="bg-bg border border-panel2 rounded px-2 py-1 w-24" />
                      ) : (e.country ?? "—")}
                    </td>
                    <td className="px-3 py-2 text-xs text-ink2">{e.type ?? "—"}</td>
                    ...
                    <td className="px-3 py-2 text-left">
                      {isEditing ? (
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => startEdit(e)} className="text-xs text-ink2 hover:text-accent">✏</button>
                          {e.active === 1 ? (
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-3 py-8 text-center text-ink2 text-sm">
                    אין תוצאות.
                  </td>
                </tr>
              )}
